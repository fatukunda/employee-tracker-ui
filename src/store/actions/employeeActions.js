...
// const baseUrl = "https://localhost:5001/api/v1/employees";
...
export const registerEmployee = (employeeDetails, handleClose) => async (
  dispatch
) => {
...
export const viewSingleemployee = (employeeId) => async (dispatch) => {
...
export const editemployee = (employeeData, id) => async (dispatch) => {
  dispatch(editemployeePending(true));
  try {
    const response = await axios.put(`${baseUrl}/${id}`, employeeData);
    const { data } = response;
...
export const searchemployees = (employees, keyword) => (dispatch) => {
